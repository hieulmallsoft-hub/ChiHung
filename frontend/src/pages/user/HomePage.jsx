  ...
  return (
    <div className="space-y-12">
      <section className="relative overflow-hidden rounded-[2rem] border border-primary-300/40 bg-gradient-to-br from-primary-700 via-primary-600 to-primary-800 p-6 text-white shadow-glow md:p-10">
        ...
        <div className="pointer-events-none absolute -right-24 -top-24 h-72 w-72 rounded-full bg-rose-200/20 blur-3xl" />

        <div className="relative grid gap-8 lg:grid-cols-[1.1fr,0.9fr] lg:items-center">
          <div className="space-y-6">
            <span className="inline-flex w-fit items-center rounded-full border border-white/40 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-rose-100">
              Sport Ecommerce + Realtime Chat
            </span>

            <h1 className="font-heading text-4xl font-extrabold leading-[1.02] md:text-6xl">
              Ban do the thao dep,
              <br />
          ...
          <div className="rounded-[1.6rem] border border-white/25 bg-white/10 p-5 backdrop-blur md:p-6">
            <p className="text-sm font-semibold text-rose-100">Danh muc duoc quan tam nhieu</p>
            <div className="mt-4 flex flex-wrap gap-2">
              {highlights.map((item, index) => (
                <span
                  key={item}
                  className={`rounded-full px-3 py-1 text-xs font-bold ${index === 0 ? "bg-white text-primary-700" : "bg-white/20 text-white"}`}
                >
                  {item}
                </span>
              ))}
            ...
            <div className="mt-5 grid grid-cols-3 gap-2 text-center">
              <div className="rounded-xl bg-white/20 px-2 py-3 backdrop-blur">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {[
          ...
          <article key={item.title} className="section-shell">
      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-rose-200/80 bg-white p-6 shadow-soft md:p-9">
        <div className="absolute -right-20 top-0 h-52 w-52 rounded-full bg-primary-100/70 blur-3xl" />
